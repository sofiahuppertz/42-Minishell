...
static int update_old_dir(t_env **env)
{
    ...
    if (!old_pwd)
    {
        old_pwd = ft_strjoin((const char *)"OLDPWD=", (const char *)current_dir);
        if (!old_pwd)
            return (ERROR); // NOT FATAL BECAUSE ENVP IS OK.
        if (envp_add_var((const char *)old_pwd, env) == ERROR) 
            exit_failure(); //FATAL BECAUSE ENVP MESSED UP.
    }
    else
    {
        if (envp_modify_var((const char *)"OLDPWD", (const char *)current_dir, env) == ERROR)
            exit_failure(); //FATAL BECAUSE ENVP MESSED UP.
    }
    free(old_pwd);
    return (SUCCESS);
...
static int		cd_home(t_env **env)
{
    ...
    update_old_dir(env);
    home_path = envp_get_value("HOME", *env); //free afterwards.
    if (!home_path)
    {
...
static int  cd_prev_dir(t_env **env)
{
    ...
    prev_path = envp_get_value("OLDPWD", *env); //free afterwards.
    if (!prev_path)
    {
